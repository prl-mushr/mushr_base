...
class RacecarState:
    ...
    def __init__(self):
        ...
        self.STEERING_ANGLE_OFFSET = float(rospy.get_param("~steering_angle_offset", 0.00))

        # Steering angle noise std dev
        self.STEERING_ANGLE_NOISE = float(rospy.get_param("~steering_angle_noise", 0.000001))
        ...
        self.joint_msg.name = ['front_left_wheel_throttle',
    ...
    def init_pose_cb(self, msg):
        ...
        rx_trans = np.array([msg.pose.pose.position.x,
                             msg.pose.pose.position.y], dtype=np.float)
        ...
        if (self.permissible_region is None or 
            self.permissible_region[int(map_rx_pose[1]+0.5), int(map_rx_pose[0]+0.5)] == 1):  
          ...
          self.cur_odom_to_base_trans[0] = offset_in_map[0]*np.cos(-self.cur_map_to_odom_rot) - offset_in_map[1]*np.sin(-self.cur_map_to_odom_rot)
          self.cur_odom_to_base_trans[1] = offset_in_map[0]*np.sin(-self.cur_map_to_odom_rot) + offset_in_map[1]*np.cos(-self.cur_map_to_odom_rot)
    ...
    def timer_cb(self, event):
        ...
        new_pose = np.array([self.cur_odom_to_base_trans[0],
                             self.cur_odom_to_base_trans[1],
                             self.cur_odom_to_base_rot], dtype=np.float)
        if np.abs(delta) < 1e-2:
            ...
            dy = (self.CAR_LENGTH / tan_delta) * (-1 * np.cos(self.cur_odom_to_base_rot + dtheta) + np.cos(self.cur_odom_to_base_rot))
        ...
        new_map_pose[0] = self.cur_map_to_odom_trans[0] + (new_pose[0]*np.cos(self.cur_map_to_odom_rot)-new_pose[1]*np.sin(self.cur_map_to_odom_rot))
        new_map_pose[1] = self.cur_map_to_odom_trans[1] + (new_pose[0]*np.sin(self.cur_map_to_odom_rot)+new_pose[1]*np.cos(self.cur_map_to_odom_rot))
        ...
        if (self.permissible_region is None or 
            ...
            for i in xrange(len(self.joint_msg.position)):
              self.joint_msg.position[i] = self.clip_angle(self.joint_msg.position[i])              
        
        # Publish the tf from odom to base_footprint
        self.br.sendTransform((self.cur_odom_to_base_trans[0], self.cur_odom_to_base_trans[1], 0.0),
